...
function Integrantes() {
  ...
  const htmlCode = listaIntegrantes.map((integrante, index) => (
    <li key={index}>
      <Link href={integrante.github}>{integrante.nombre}</Link>
    </li>
  ));
  return htmlCode;
}
...
export default function Home() {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen py-2">
      <img src="/img/logo.png" alt="MusicPro Logo" width={200} height={200} />
      ...
      <div className="flex flex-wrap items-center justify-around max-w-4xl mt-6 sm:w-full">
        <Link href="/login" className="p-6 mt-6 text-left border w-96 rounded-xl transition duration-500 hover:shadow-2xl hover:shadow-red-800 hover:text-red-800 focus:text-red-800">
